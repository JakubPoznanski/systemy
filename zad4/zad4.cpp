...
int main(int argc, char *argv[])
{
    int bytesForBufP = 3;
    int bytesForBufC = 3;
    int id, filedes[2], bytesread, inFileDescriptor, outFileDescriptor, random_value;
    ...
    case 0:
        ...
        do
        {
            ...
            bytesread = read(filedes[0], &bufferC, bytesForBufC);
            // Wpis do pliku wyjściowego
            if (write(outFileDescriptor, &bufferC, bytesread) == -1)
            {
                perror("write error\n");
                exit(0);
            }
            // wpis bufora na standardowe wyjście
            if (write(STDOUT_FILENO, &bufferC, bytesread) == -1)
            {
                perror("write error\n");
                exit(0);
            }
            sprintf(bytesNumber, "%d", bytesread);
            if (write(STDOUT_FILENO, &bytesNumber, bytesread) == -1)
            {
                perror("write error\n");
                exit(EXIT_FAILURE);
            }
        } while (bytesread == bytesForBufC);
    ...
    default:
        ...
        do
        {
            ...
            bytesread = read(inFileDescriptor, &bufferP, bytesForBufP);
            // wpis z bufora do wejscia potoku
            if (write(filedes[1], &bufferP, bytesread) == -1)
            {
                perror("write error\n");
                exit(0);
            }
            // wpis bufora na standardowe wyjście
            if (write(STDOUT_FILENO, &bufferP, bytesread) == -1)
            {
                perror("write error\n");
                exit(0);
            }

        } while (bytesread == bytesForBufP);
